...
fn should_execute_and_fetch() -> Result<(), C3p0Error> {
    test(async {
        ...
        pool.transaction(|mut conn| async move {
            ...
            assert_eq!(
                0,
                conn.fetch_one_value::<i64>(&format!("SELECT COUNT(*) FROM {}", table_name), &[]).await
                    .unwrap()
            );
            ...
            assert!(conn
                .execute(&format!(r"DROP TABLE {}", table_name), &[]).await
                .is_ok());
            Ok(())
        }).await
    })
...
fn should_fetch_values() -> Result<(), C3p0Error> {
    test(async {
        ...
        let result: Result<_, C3p0Error> = pool.transaction(|mut conn| async move {
            let conn = &mut conn;
            assert!(conn
            ...
            let all_string =
                conn.fetch_all_values::<String>(&format!("SELECT * FROM {}", table_name), &[]).await;
            assert!(all_string.is_ok());
            assert!(all_string.unwrap().is_empty());

            let one_string =
                conn.fetch_one_value::<String>(&format!("SELECT * FROM {}", table_name), &[]).await;
            assert!(one_string.is_err());

            let one_i64 = conn.fetch_one_value::<i64>(&format!("SELECT * FROM {}", table_name), &[]).await;
            ...
            let all_i64 = conn.fetch_all_values::<i64>(&format!("SELECT * FROM {}", table_name), &[]).await;
            ...
            assert!(conn
                .batch_execute(&format!("DROP TABLE {}", table_name)).await
                .is_ok());
