    ...
    test(async {
...

        {
            pool.transaction::<_, C3p0Error, _ ,_>(|mut conn| async move {
                let conn = &mut conn;
                let count = conn
                    .fetch_one_value::<i64>(&format!(r"SELECT COUNT(*) FROM {}", table_name), &[]).await
                    .unwrap();
                assert_eq!(3, count);
...
fn should_rollback_transaction() {
    test(async {
        ...
        let result_create_table: Result<(), C3p0Error> = c3p0.transaction(|mut conn| async move {
            let conn = &mut conn;
            assert!(conn
        ...
        let result: Result<(), C3p0Error> = c3p0.transaction(|mut conn| async move {
            let conn = &mut conn;
            conn.execute(
                &format!(r"INSERT INTO {} (name) VALUES ('one')", table_name),
                &[],
...

        {
            pool.transaction::<_, C3p0Error, _ ,_>(|mut conn| async move {
                ...
                assert!(conn
                    .execute(&format!(r"DROP TABLE IF EXISTS {}", table_name), &[]).await
                    .is_ok());
                Ok(())
            }).await
                .unwrap();
...
fn transaction_should_return_internal_error() {
    test(async {
        ...
        let result: Result<(), _> = c3p0.transaction(|_| async move {
