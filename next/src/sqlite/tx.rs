...
impl Tx<Sqlite> for SqliteConnection {
    ...
    async fn fetch_all_with_sql<'a, DATA: Data, A: 'a + Send + sqlx::IntoArguments<'a, Sqlite>>(
        &mut self,
        sql: sqlx::query::Query<'a, Sqlite, A>,
    ) -> Result<Vec<Record<DATA>>, C3p0Error> {
        <Record::<DATA> as DbRead<Sqlite, DATA>>::fetch_all_with_sql(self, sql).await
    }

    async fn fetch_one_optional_with_sql<'a, DATA: Data, A: 'a + Send + sqlx::IntoArguments<'a, Sqlite>>(
        &mut self,
        sql: sqlx::query::Query<'a, Sqlite, A>,
    ) -> Result<Option<Record<DATA>>, C3p0Error> {
         <Record::<DATA> as DbRead<Sqlite, DATA>>::fetch_one_optional_with_sql(self, sql).await
    }

    async fn fetch_one_with_sql<'a, DATA: Data, A: 'a + Send + sqlx::IntoArguments<'a, Sqlite>>(
        &mut self,
        sql: sqlx::query::Query<'a, Sqlite, A>,
    ) ->  Result<Record<DATA>, C3p0Error> {
        <Record::<DATA> as DbRead<Sqlite, DATA>>::fetch_one_with_sql(self, sql).await
    }

    async fn count_all<DATA: Data>(&mut self) -> Result<u64, C3p0Error> {
        <Record::<DATA> as DbRead<Sqlite, DATA>>::count_all(self).await
    }

    async fn exists_by_id<DATA: Data>(&mut self, id: u64) -> Result<bool, C3p0Error> {
        <Record::<DATA> as DbRead<Sqlite, DATA>>::exists_by_id(self, id).await
    }

    async fn fetch_all<DATA: Data>(&mut self) -> Result<Vec<Record<DATA>>, C3p0Error> {
        <Record::<DATA> as DbRead<Sqlite, DATA>>::fetch_all(self).await
    }
    ...
    async fn fetch_one_optional_by_id<DATA: Data>(
        ...
        <Record::<DATA> as DbRead<Sqlite, DATA>>::fetch_one_optional_by_id(self, id).await
    }

    async fn fetch_one_by_id<DATA: Data>(&mut self, id: u64) -> Result<Record<DATA>, C3p0Error> {
        <Record::<DATA> as DbRead<Sqlite, DATA>>::fetch_one_by_id(self, id).await
    }
    ...
    async fn delete<DATA: Data>(
        ...
        <Record::<DATA> as DbRead<Sqlite, DATA>>::delete(record, self).await
    }

    async fn delete_all<DATA: Data>(&mut self) -> Result<u64, C3p0Error> {
        <Record::<DATA> as DbRead<Sqlite, DATA>>::delete_all(self).await
    }

    async fn delete_by_id<DATA: Data>(&mut self, id: u64) -> Result<u64, C3p0Error> {
        <Record::<DATA> as DbRead<Sqlite, DATA>>::delete_by_id(self, id).await
    }
    ...
    async fn update<DATA: Data>(
        ...
        <Record::<DATA> as DbRead<Sqlite, DATA>>::update(record, self).await
    }

    async fn save<DATA: Data>(&mut self, record: NewRecord<DATA>) -> Result<Record<DATA>, C3p0Error> {
        <NewRecord::<DATA> as DbWrite<Sqlite, DATA>>::save(record, self).await
    }
