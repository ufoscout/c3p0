...
pub trait PgC3p0MigrateBuilder {
    fn build(self) -> C3p0Migrate<SqlxConnection, SqlxC3p0Pool, PgMigrator>;
}

impl PgC3p0MigrateBuilder for C3p0MigrateBuilder<SqlxC3p0Pool> {
    fn build(self) -> C3p0Migrate<SqlxConnection, SqlxC3p0Pool, PgMigrator> {
        C3p0Migrate::new(
            self.table,
            self.schema,
...
impl Migrator for PgMigrator {
    ...
    type C3P0 = SqlxC3p0Pool;
    type C3P0Json = SqlxC3p0Json<MigrationData, DefaultJsonCodec>;

    fn build_cp30_json(
        &self,
        table: String,
        schema: Option<String>,
    ) -> SqlxC3p0Json<MigrationData, DefaultJsonCodec> {
        C3p0JsonBuilder::<SqlxC3p0Pool>::new(table)
    ...
    async fn lock_table(
        &self,
        c3p0_json: &SqlxC3p0Json<MigrationData, DefaultJsonCodec>,
