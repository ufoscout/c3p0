...
pub trait MySqlIdGenerator<Id: IdType, DbId: MySqlIdType>: Clone + Send + Sync + IdGenerator<Id, DbId> {
    fn from_inserted_id(&self, inserted_id: u64) -> Result<DbId, C3p0Error>;
}
...
impl MySqlIdGenerator<u64, u64> for MySqlAutogeneratedIdGenerator {
    fn from_inserted_id(&self, inserted_id: u64) -> Result<u64, C3p0Error> {
        Ok(inserted_id)
    }
}

impl IdGenerator<u64, u64> for MySqlAutogeneratedIdGenerator {
    fn generate_id(&self) -> Option<u64> {
        None
    }

    fn from_id_to_db_id<'a>(&self, id: Cow<'a, u64>) -> Result<Cow<'a, u64>, C3p0Error> {
        Ok(id)
    }

    fn from_db_id_to_id<'a>(&self, id: Cow<'a, u64>) -> Result<Cow<'a, u64>, C3p0Error> {
        Ok(id)
    }
}
...
impl MySqlIdGenerator<uuid::Uuid, uuid::Uuid> for MySqlUuidIdGenerator{
    fn from_inserted_id(&self, _inserted_id: u64) -> Result<uuid::Uuid, C3p0Error> {
...
impl IdGenerator<uuid::Uuid, uuid::Uuid> for MySqlUuidIdGenerator {
...
#[derive(Clone)]
pub struct SqlxMySqlC3p0JsonBuilder<Id: IdType, DbId: MySqlIdType, Generator: MySqlIdGenerator<Id, DbId>> {
...
impl SqlxMySqlC3p0JsonBuilder<u64, u64, MySqlAutogeneratedIdGenerator> {
    pub fn new<T: Into<String>>(table_name: T) -> Self {
        ...
        SqlxMySqlC3p0JsonBuilder {
            ...
            id_generator: MySqlAutogeneratedIdGenerator {},
            table_name,
...
impl<Id: IdType, DbId: MySqlIdType, Generator: MySqlIdGenerator<Id, DbId>> SqlxMySqlC3p0JsonBuilder<Id, DbId, Generator> {
    ...
    pub fn with_id_generator<NewId: IdType, NewDbId: MySqlIdType, NewGenerator: MySqlIdGenerator<NewId, NewDbId>>(
        self,
        id_generator: NewGenerator,
    ) -> SqlxMySqlC3p0JsonBuilder<NewId, NewDbId, NewGenerator> {
        SqlxMySqlC3p0JsonBuilder {
            phantom_id: std::marker::PhantomData,
            phantom_db_id: std::marker::PhantomData,
            id_generator,
    ...
    pub fn build<Data: DataType>(self) -> SqlxMySqlC3p0Json<Id, DbId, Generator, Data, DefaultJsonCodec> {
    ...
    pub fn build_with_codec<Data: DataType, CODEC: JsonCodec<Data>>(
        self,
        codec: CODEC,
    ) -> SqlxMySqlC3p0Json<Id, DbId, Generator, Data, CODEC> {
        SqlxMySqlC3p0Json {
...
#[derive(Clone)]
pub struct SqlxMySqlC3p0Json<Id: IdType, DbId: MySqlIdType, Generator: MySqlIdGenerator<Id, DbId>, Data: DataType, CODEC: JsonCodec<Data>> {
...
impl<Id: IdType, DbId: MySqlIdType, Generator: MySqlIdGenerator<Id, DbId>, Data: DataType, CODEC: JsonCodec<Data>> SqlxMySqlC3p0Json<Id, DbId, Generator, Data, CODEC> {
...
#[async_trait]
impl<Id: IdType, DbId: MySqlIdType, Generator: MySqlIdGenerator<Id, DbId>, Data: DataType, CODEC: JsonCodec<Data>> C3p0Json<Id, Data, CODEC>
    for SqlxMySqlC3p0Json<Id, DbId, Generator, Data, CODEC>
{
    type Tx = MySqlTx;
    ...
    async fn save(
        ...
        let id = if let Some(id) = self.id_generator.generate_id() {
            ...
            let id = sqlx::query(&self.queries.save_sql_query)
                ...
                .map_err(into_c3p0_error)?;
            self.id_generator.from_inserted_id(id)?
        };
