...
pub trait MySqlIdGenerator<Id: IdType>: IdGenerator<Id, Db = Db, Row = DbRow>
{
    ...
    fn as_trait_a(&self) -> &dyn IdGenerator<Id, Db = Db, Row = DbRow>;
}
...
impl MySqlIdGenerator<u64> for MySqlAutogeneratedIdGenerator {
    ...
    fn as_trait_a(&self) -> &dyn IdGenerator<u64, Db = Db, Row = DbRow> {
        self
    }
}
...
impl MySqlIdGenerator<uuid::Uuid> for MySqlUuidIdGenerator {
    ...
    fn as_trait_a(&self) -> &dyn IdGenerator<uuid::Uuid, Db = Db, Row = DbRow> {
        self
    }
}
    ...
    > SqlxMySqlC3p0Json<Id, Data, CODEC>
{
    ...
    pub fn to_model(&self, row: &DbRow) -> Result<Model<Id, Data>, C3p0Error> {
        to_model(&self.codec, self.id_generator.as_trait_a(), row, 0, 1, 2, 3, 4)
    }
    ...
    pub async fn fetch_one_optional_with_sql<'a, A: 'a + Send + IntoArguments<'a, Db>>(
        ...
        sql.fetch_optional(tx.conn())
            ...
            .map(|row| to_model(&self.codec, self.id_generator.as_trait_a(), &row, 0, 1, 2, 3, 4))
            .transpose()
    }
    ...
    pub async fn fetch_one_with_sql<'a, A: 'a + Send + IntoArguments<'a, Db>>(
        ...
        sql.fetch_one(tx.conn())
            .await
            .map_err(into_c3p0_error)
            .and_then(|row| to_model(&self.codec, self.id_generator.as_trait_a(), &row, 0, 1, 2, 3, 4))
    }
