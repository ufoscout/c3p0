...
impl IdGenerator<u64> for PostgresAutogeneratedIdGenerator {
    ...
    fn id_to_query<'a>(&self, id: &'a u64, query: Query<'a, Db, <Db as HasArguments<'a>>::Arguments>) -> Query<'a, Db, <Db as HasArguments<'a>>::Arguments> {
...
impl IdGenerator<uuid::Uuid> for PostgresUuidIdGenerator {
    ...
    fn id_to_query<'a>(&self, id: &'a uuid::Uuid, query: Query<'a, Db, <Db as HasArguments<'a>>::Arguments>) -> Query<'a, Db, <Db as HasArguments<'a>>::Arguments> {
        query.bind(id)
    }
    ...
    > C3p0Json<Id, Data, CODEC> for SqlxPgC3p0Json<Id, Data, CODEC>
{
    ...
    async fn exists_by_id<'a, ID: Into<&'a Id> + Send>(
        ...
        self.id_generator.id_to_query(id.into(), query)
            .fetch_one(tx.conn())
            .await
            .and_then(|row| row.try_get(0))
            .map_err(into_c3p0_error)
    ...
    async fn delete(
        ...
        let result = self.id_generator.id_to_query(&obj.id, query)
    ...
    async fn update(
        ...
        let result = {
            let query = sqlx::query(&self.queries.update_sql_query)
                ...
                .bind(json_data);
            self.id_generator.id_to_query(&updated_model.id, query)
