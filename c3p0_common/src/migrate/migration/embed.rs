    ...
    for entry in dir.dirs() {
        ...
        let id = entry
            ...
            .ok_or_else(|| C3p0Error::IoError {
                cause: format!("Cannot get filename of [{}]", entry.path().display()),
            })?;
...
mod test {
    ...
    fn should_return_error_if_missing_down_sql() {
        ...
        match migrations {
            Err(C3p0Error::IoError { cause }) => {
                assert!(cause.contains("Error reading file [00010_create_test_data/down.sql]"));
            }
            _ => panic!(),
        };
    }
    ...
    fn should_return_error_if_missing_up_sql() {
        ...
        match migrations {
            Err(C3p0Error::IoError { cause }) => {
                assert!(cause.contains("Error reading file [00010_create_test_data/up.sql]"));
            }
            _ => panic!(),
        };
    }
