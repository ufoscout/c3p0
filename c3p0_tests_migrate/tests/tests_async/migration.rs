...
#[tokio::test]
async fn should_create_the_c3p0_migrate_table_with_default_name() -> Result<(), Box<dyn std::error::Error>>
{
...
#[tokio::test]
async fn should_create_the_c3p0_migrate_table_with_custom_name() -> Result<(), Box<dyn std::error::Error>>
{
    let docker = clients::Cli::default();
    let node = new_connection(&docker).await;
...
async fn should_read_migrations_from_files() -> Result<(), Box<dyn std::error::Error>> {
    ...
    node.0.transaction(|mut conn| async move {
        ...
        assert_eq!(
            3,
            conn.fetch_one_value::<i64>(&format!("select count(*) from TEST_TABLE"), &[]).await?
        );
