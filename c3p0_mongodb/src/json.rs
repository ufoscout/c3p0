...
pub trait IdGenerator<Id: IdType, MongodbId: MongodbIdType>: Send + Sync {
    fn generate_id(&self) -> Option<MongodbId>;
    fn from_id_to_mongoid<'a>(&self, id: Cow<'a, Id>) -> Result<Cow<'a, MongodbId>, C3p0Error>;
    fn from_mongoid_to_id<'a>(&self, id: Cow<'a, MongodbId>) -> Result<Cow<'a, Id>, C3p0Error>;
    fn from_inserted_id(&self, inserted_id: mongodb::bson::Bson) -> Result<MongodbId, C3p0Error>;
}
...
impl IdGenerator<ObjectId, ObjectId> for AutogeneratedIdGenerator {
    ...
    fn from_id_to_mongoid<'a>(&self, id: Cow<'a, ObjectId>) -> Result<Cow<'a, ObjectId>, C3p0Error> {
        Ok(id)
    }

    fn from_mongoid_to_id<'a>(&self, id: Cow<'a, ObjectId>) -> Result<Cow<'a, ObjectId>, C3p0Error> {
        Ok(id)
    }
...
impl IdGenerator<uuid::Uuid, uuid::Uuid> for UuidIdGenerator {
    ...
    fn from_id_to_mongoid<'a>(&self, id: Cow<'a, uuid::Uuid>) -> Result<Cow<'a, uuid::Uuid>, C3p0Error> {
        Ok(id)
    }

    fn from_mongoid_to_id<'a>(&self, id: Cow<'a, uuid::Uuid>) -> Result<Cow<'a, uuid::Uuid>, C3p0Error> {
        Ok(id)
    }
...
#[derive(Clone)]
pub struct MongodbC3p0JsonBuilder<Id, MongodbId: MongodbIdType> {
    pub id_generator: Arc<dyn IdGenerator<Id, MongodbId>>,
...
impl<Id: IdType, MongodbId: MongodbIdType> MongodbC3p0JsonBuilder<Id, MongodbId> {
    pub fn with_id_generator<NewId: IdType, NewMongodbId: MongodbIdType, T: 'static + IdGenerator<NewId, NewMongodbId> + Send + Sync>(
        self,
        id_generator: T,
    ) -> MongodbC3p0JsonBuilder<NewId, NewMongodbId> {
        MongodbC3p0JsonBuilder {
    ...
    pub fn build<Data: DataType>(self) -> MongodbC3p0Json<Id, MongodbId, Data, DefaultJsonCodec> {
    ...
    pub fn build_with_codec<Data: DataType, CODEC: JsonCodec<Data>>(
        self,
        codec: CODEC,
    ) -> MongodbC3p0Json<Id, MongodbId, Data, CODEC> {
        MongodbC3p0Json {
...
#[derive(Clone)]
pub struct MongodbC3p0Json<Id: IdType, MongodbId: MongodbIdType, Data: DataType, CODEC: JsonCodec<Data>> {
    phantom_data: std::marker::PhantomData<Data>,
    id_generator: Arc<dyn IdGenerator<Id, MongodbId>>,
    codec: CODEC,
    table_name: String,
}

#[async_trait]
impl<Id: IdType, MongodbId: MongodbIdType, Data: DataType, CODEC: JsonCodec<Data>> C3p0Json<Id, Data, CODEC>
    for MongodbC3p0Json<Id, MongodbId, Data, CODEC> {
    type Tx = MongodbTx;
    ...
    async fn count_all(&self, tx: &mut MongodbTx) -> Result<u64, C3p0Error> {
        ...
        db.collection::<ModelWithId<MongodbId>>(&self.table_name)
    ...
    async fn exists_by_id<'a, ID: Into<&'a Id> + Send>(
        ...
        let filter = doc! { "_id": self.id_generator.from_id_to_mongoid(Cow::Borrowed(id.into()))?.as_ref() };
    ...
    async fn fetch_all(&self, tx: &mut MongodbTx) -> Result<Vec<Model<Id, Data>>, C3p0Error> {
        let (db, session) = tx.db();
        let mut cursor = db
            .collection::<ModelWithId<MongodbId>>(&self.table_name)
    ...
    async fn fetch_one_optional_by_id<'a, ID: Into<&'a Id> + Send>(
        ...
        let filter = doc! {
            "_id": self.id_generator.from_id_to_mongoid(Cow::Borrowed(id.into()))?.as_ref()
        };
        let (db, session) = tx.db();
        let model = db
            .collection::<ModelWithId<MongodbId>>(&self.table_name)
    ...
    async fn delete(
        ...
        let filter = doc! {
            "_id": self.id_generator.from_id_to_mongoid(Cow::Borrowed(&obj.id))?.as_ref(),
            "version": obj.version
        };
        let (db, session) = tx.db();
        let result = db
...
impl<MongodbId: MongodbIdType> ModelWithId<MongodbId> {

    fn into_model<Id: IdType, Data: DataType, Codec: JsonCodec<Data>>(
        self,
        id_generator: &(dyn IdGenerator<Id, MongodbId>),
        codec: &Codec,
    ) -> Result<Model<Id, Data>, C3p0Error> {
        Ok(Model {
            id: id_generator.from_mongoid_to_id(Cow::Owned(self.id))?.into_owned(),
    ...
    fn from_model<Id: IdType, Data: DataType, Codec: JsonCodec<Data>>(
        ...
        Ok(ModelWithId {
            id: id_generator.from_id_to_mongoid(Cow::Owned(model.id))?.into_owned(),
