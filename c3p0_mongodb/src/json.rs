...
impl IdGenerator<ObjectId, ObjectId> for AutogeneratedIdGenerator {
    ...
    fn id_to_db_id<'a>(&self, id: Cow<'a, ObjectId>) -> Result<Cow<'a, ObjectId>, C3p0Error> {
        Ok(id)
    }

    fn db_id_to_id<'a>(&self, id: Cow<'a, ObjectId>) -> Result<Cow<'a, ObjectId>, C3p0Error> {
        Ok(id)
    }

    fn inserted_id_to_db_id(
        &self,
        inserted_id: mongodb::bson::Bson,
...
impl IdGenerator<uuid::Uuid, mongodb::bson::Uuid> for UuidIdGenerator {
    fn generate_id(&self) -> Option<mongodb::bson::Uuid> {
        Some(mongodb::bson::Uuid::new())
    }

    fn id_to_db_id<'a>(
        &self,
        id: Cow<'a, uuid::Uuid>,
    ) -> Result<Cow<'a, mongodb::bson::Uuid>, C3p0Error> {
        let id = id.into_owned();
        Ok(Cow::Owned(mongodb::bson::Uuid::from(id)))
    }

    fn db_id_to_id<'a>(
        &self,
        id: Cow<'a, mongodb::bson::Uuid>,
    ) -> Result<Cow<'a, uuid::Uuid>, C3p0Error> {
        let id = id.into_owned();
        Ok(Cow::Owned(id.into()))
    }

    fn inserted_id_to_db_id(
        &self,
        _inserted_id: mongodb::bson::Bson,
    ) -> Result<mongodb::bson::Uuid, C3p0Error> {
        Err(C3p0Error::RowMapperError {
    ...
    C3p0Json<Id, Data, CODEC> for MongodbC3p0Json<Id, DbId, Data, CODEC>
{
    ...
    async fn exists_by_id<'a, ID: Into<&'a Id> + Send>(
        ...
        let filter =
            doc! { "_id": self.id_generator.id_to_db_id(Cow::Borrowed(id.into()))?.as_ref() };
    ...
    async fn fetch_one_optional_by_id<'a, ID: Into<&'a Id> + Send>(
        ...
        let filter = doc! {
            "_id": self.id_generator.id_to_db_id(Cow::Borrowed(id.into()))?.as_ref()
        };
        let (db, session) = tx.db();
        let model = db
            .collection::<ModelWithId<DbId>>(&self.table_name)
    ...
    async fn delete(
        ...
        let filter = doc! {
            "_id": self.id_generator.id_to_db_id(Cow::Borrowed(&obj.id))?.as_ref(),
            "version": obj.version
        };
        let (db, session) = tx.db();
        let result = db
    ...
    async fn save(
        ...
        let new_model = if let Some(id) = self.id_generator.generate_id() {
            ...
            let result = db
                ...
                .await
                .map_err(into_c3p0_error)?;
            let id = self.id_generator.inserted_id_to_db_id(result.inserted_id)?;
            ModelWithId {
                id,
...
impl<DbId: MongodbIdType> ModelWithId<DbId> {
    fn into_model<Id: IdType, Data: DataType, Codec: JsonCodec<Data>>(
        ...
        Ok(Model {
            id: id_generator.db_id_to_id(Cow::Owned(self.id))?.into_owned(),
    ...
    fn from_model<Id: IdType, Data: DataType, Codec: JsonCodec<Data>>(
        ...
        Ok(ModelWithId {
            id: id_generator.id_to_db_id(Cow::Owned(model.id))?.into_owned(),
