...
impl IdGenerator<ObjectId> for AutogeneratedIdGenerator {
    ...
    fn id_to_db_id(&self, id: Cow<'_, ObjectId>) -> Result<Bson, C3p0Error> {
        Ok(id.as_ref().into())
    }

    fn db_id_to_id(&self, id: &mongodb::bson::Bson) -> Result<ObjectId, C3p0Error> {
...
impl IdGenerator<uuid::Uuid> for UuidIdGenerator {
    ...
    fn id_to_db_id(&self, id: Cow<'_, uuid::Uuid>) -> Result<Bson, C3p0Error> {
        let id = id.into_owned();
        Ok(mongodb::bson::Uuid::from(id).into())
    }

    fn db_id_to_id(&self, id: &Bson) -> Result<uuid::Uuid, C3p0Error> {
        match id {
            mongodb::bson::Bson::Binary(binary) => {
                if let mongodb::bson::spec::BinarySubtype::Uuid = binary.subtype {
                    return binary.clone().bytes.try_into().map_err(|err| {
