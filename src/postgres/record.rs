...
impl<DATA: DataType> DbRead<Postgres, DATA> for Record<DATA> {
    ...
    async fn fetch_all(tx: &mut PgConnection) -> Result<Vec<Record<DATA>>, C3p0Error> {
        ...
        Self::fetch_all_with_sql(tx, sqlx::query::<Postgres>(&query)).await
    }
    ...
    async fn fetch_one_optional_by_id(
        ...
        let query = sqlx::query::<Postgres>(&query).bind(id as i64);
...
impl<DATA: DataType> DbWrite<Postgres, DATA> for NewRecord<DATA> {
